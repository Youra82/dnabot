#!/usr/bin/env python3
# run_manual_portfolio_momentum_exit.py
# Manuelle Portfolio-Simulation fuer momentum_exit: Nutzer waehlt Pairs aus
# den backtest_momentum_exit.py-Ergebnissen, Bot simuliert kombinierten
# Kapital-Pool und exportiert optional Excel-Trade-Log + HTML-Equity-Chart.
#
# Wiederbelebung des fruaheren run_manual_portfolio.py (Genome-System, beim
# Cleanup 2026-08-24 entfernt) -- identisches Excel/HTML-Format, aber als
# Datenquelle die momentum_exit-Backtest-Ergebnisse (backtest_*_momentum_
# exit.json, siehe backtest_momentum_exit.py) statt der Genome-Backtests.
...
def _clean_timeframe(tf: str) -> str:
    """backtest_momentum_exit.py speichert das Timeframe-Feld als z.B.
    '6h_momentum_exit' (siehe save_results()-Aufruf dort) -- fuer die
    Anzeige den Suffix abschneiden."""
...
def load_all_results(start_date=None, end_date=None):
    ...
    for fname in sorted(os.listdir(RESULTS_DIR)):
        ...
        results.append({
            'market':    data['market'],
            'timeframe': _clean_timeframe(data['timeframe']),
            'trades':    trades,
        })
...
def select_pairs(all_results, capital, risk_pct):
    """Zeigt alle Pairs mit PnL% und laesst Nutzer auswaehlen."""
    pairs_with_stats = []
    ...
    w = 72
    print(f"\n{'=' * w}")
    print(f"{B}  Verfuegbare Pairs (momentum_exit){NC}")
    ...
    try:
        ...
        print(f"  {R}Ungueltige Eingabe.{NC}")
        return []
...
def build_telegram_report(selected, pm, capital, risk_pct, start_date, end_date):
    ...
    lines = [
        "dnabot — Manuelle Portfolio-Simulation (momentum_exit)",
        f"Zeitraum: {date_range}",
        f"Kapital: {capital:.0f} USDT | Risiko: {risk_pct}%",
        "",
        f"Ausgewaehlte Pairs ({len(selected)}):",
    ]
    for pr in selected:
        st = pr['stats']
...
def print_result(selected, pm, capital, risk_pct, start_date, end_date):
    ...
    date_range = f" | {start_date or '...'} → {end_date or 'heute'}"
    print(f"\n{'=' * w}")
    print(f"{B}  dnabot — Manuelle Portfolio-Simulation (momentum_exit){NC}")
    print(f"  Zeitraum:{date_range}")
    print(f"  Kapital: {capital:.0f} USDT | Risiko/Trade: {risk_pct}% (gemeinsamer Pool)")
    print(f"{'=' * w}")

    print(f"\n  {G}Ausgewaehlte Pairs — {len(selected)} Pair(s){NC}")
...
def send_telegram(report_text):
    ...
    if not bot_token or not chat_id:
        print(f"  {Y}Kein Telegram-Token/Chat-ID in secret.json — uebersprungen.{NC}")
        return
...
def generate_portfolio_equity_chart(selected: list, pm: dict,
    ...
    try:
        ...
        print(f"{R}  plotly nicht installiert — Chart uebersprungen. (pip install plotly){NC}")
        return
    ...
    if not all_trades:
        print(f"  {Y}Keine Trades vorhanden — Chart uebersprungen.{NC}")
        return
    ...
    title = (
        f"dnabot momentum_exit Portfolio — {len(selected)} Coins ({pairs_str}) | "
        f"Trades: {n} | WR: {wr:.1%} | "
    ...
    for i, t in enumerate(all_trades):
        ...
        entry_y.append(eq_val)
        entry_txt.append(tip)
        if t['outcome'] == 'WIN':
    ...
    output_file = os.path.join(tmp_dir, 'dnabot_portfolio_equity_momentum_exit.html')
    ...
    if bot_token and chat_id:
        ...
        caption = (
            f"dnabot momentum_exit Portfolio-Equity\n"
...
def generate_trades_excel(selected: list, pm: dict, capital: float, risk_pct: float,
    ...
    for i, t in enumerate(all_trades):
        ...
        ergebnis = 'Trailing/TP erreicht' if t['outcome'] == 'WIN' else ('SL erreicht' if t['outcome'] == 'LOSS' else 'Timeout')
        rows.append({
            'Nr':                    i + 1,
    ...
    tmp_dir = os.path.join(PROJECT_ROOT, 'artifacts', 'tmp')
    os.makedirs(tmp_dir, exist_ok=True)
    output_file = os.path.join(tmp_dir, 'dnabot_trades_momentum_exit.xlsx')
    wb.save(output_file)
...
def write_to_settings(selected: list, risk_pct: float = None):
    ...
    new_strategies = [
        {
            "symbol": pr['market'], "timeframe": pr['timeframe'],
    ...
    try:
        ...
        print(f"{Y}  Hinweis: risk_overrides fehlen (nur strategy_type/momentum_exit_overrides gesetzt) --{NC}")
        print(f"{Y}  live werden die Werte aus dem aktiven Risiko-Gen der RiskGenomeDB genommen, nicht{NC}")
        print(f"{Y}  aus dieser Datei. Ohne aktives Gen fuer ein Pair/Timeframe wird nicht gehandelt.{NC}\n")
        return True
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="dnabot Manuelle Portfolio-Simulation (momentum_exit)")
    ...
    if not with_trades:
        ...
        print("  Zuerst ./run_momentum_exit_pipeline.sh (oder backtest_momentum_exit.py) ausfuehren!\n")
        sys.exit(1)
    ...
    if not selected:
        print(f"\n  {Y}Keine Pairs ausgewaehlt. Abbruch.{NC}\n")
        sys.exit(0)
    ...
    try:
        settings_ans = input("  Sollen die ausgewaehlten Pairs in settings.json eingetragen werden? (j/n): ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        settings_ans = 'n'
        ...
        if excel_file:
            ...
            if bot_token and chat_id:
                ...
                send_document(
                    ...
                    caption=f"dnabot momentum_exit Trades-Tabelle | {len(selected)} Coins | "
