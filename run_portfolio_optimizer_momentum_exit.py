#!/usr/bin/env python3
# run_portfolio_optimizer_momentum_exit.py
# Automatische Portfolio-Optimierung fuer momentum_exit (jaegerbot-Style):
#
# Kapital-Modell:
...
#
# Optimizer:
#   - Greedy-Suche: bester Einzel-Kandidat pro Coin, dann iterativ weitere
#     Coins hinzufuegen solange der Calmar-Score steigt
...
#
# Wiederbelebung des fruaheren run_portfolio_optimizer.py (Genome-System,
# beim Cleanup 2026-08-24 entfernt) -- Algorithmus/Format identisch, aber
# Datenquelle sind die momentum_exit-Backtest-Ergebnisse (backtest_*_
# momentum_exit.json, siehe backtest_momentum_exit.py) statt Genome-
# Backtests. Die alte Genome/momentum_exit-Koexistenz-Logik in
# write_to_settings() entfaellt, weil es kein Genome-System mehr gibt.
...
def _clean_timeframe(tf: str) -> str:
    ...
    '6h_momentum_exit' (siehe save_results()-Aufruf dort)."""
...
def simulate_portfolio(pair_results: list, capital: float, risk_pct: float,
    ...
    for t in all_trades:
        ...
        if outcome == 'LOSS':
            pnl = -risk_amount
        else:  # WIN oder TIMEOUT: tatsaechlich simulierte Bewegung, nicht pauschales RR
...
def _calmar(metrics: dict) -> float:
    """Calmar Ratio: PnL% / MaxDD% (risiko-adjustierter Score)."""
    if metrics['max_dd'] > 0:
...
def optimize_portfolio(candidates: list, capital: float, risk_pct: float,
    ...
    -- "war zuletzt gut" (reines Calmar-Chasing) ist kaum von Zufall zu
    ...
    print(f"  1/2 Star-Spieler: {best_team[0]['market']} {best_team[0]['timeframe']} "
          f"(Calmar: {best_score:.2f})")
    print(f"  2/2 Suche beste Team-Kollegen "
...
def print_result(selected: list, pm: dict, capital: float, risk_pct: float,
    ...
    w = 72
    print(f"\n{'=' * w}")
    print(f"{B}  dnabot — Automatische Portfolio-Optimierung (momentum_exit){NC}")
...
def generate_portfolio_equity_chart(selected: list, pm: dict,
    ...
    for t in all_trades:
        ...
        if t['outcome'] == 'LOSS':
            ...
            pnl = risk_amount * (t['pnl_pct'] / sl_pct)
        if fee_pct_local:
            pnl -= (risk_amount / (sl_pct / 100.0)) * (fee_pct_local / 100.0) * 2.0
        equity += pnl
        if equity > peak:
            peak = equity
    ...
    for idx, pr in enumerate(selected):
        ...
        for t in pair_trades:
            ...
            if out == 'LOSS':
                ...
                p = ra * (t.get('pnl_pct', 0.0) / slp)
            if fee_pct_local:
                p -= (ra / (slp / 100.0)) * (fee_pct_local / 100.0) * 2.0
            peq += p
    ...
    title = (
        f"dnabot momentum_exit Optimizer-Portfolio — {len(selected)} Coins ({pairs_str}) | "
        f"Trades: {n} | WR: {wr:.1%} | "
    ...
    if bot_token and chat_id:
        ...
        caption = (
            f"dnabot momentum_exit Optimizer-Portfolio-Equity\n"
...
def write_to_settings(selected: list, risk_pct: float = None):
    ...
    # Bestehende manuelle risk_overrides/momentum_exit_overrides pro
    # (symbol, timeframe) erhalten -- sonst ginge z.B. ein manuell gesetztes
    # margin_mode/use_longs bei jedem automatischen Optimizer-Lauf verloren.
...
def main():
    parser = argparse.ArgumentParser(description="dnabot Portfolio Optimizer (momentum_exit)")
    ...
    parser.add_argument('--fee-pct', type=float, default=FEE_PCT_PER_SIDE,
                         help=f"Bitget-Taker-Gebuehr pro Seite in %% (Standard: {FEE_PCT_PER_SIDE}). "
                              "--fee-pct 0 fuer den gebuehrenfreien Vergleich.")
    ...
    if not args.auto_write:
        try:
            ...
            chart_ans = 'n'
        do_charts = chart_ans in ('j', 'ja', 'y', 'yes')
